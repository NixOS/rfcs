...
* `flakes`: ([on hold](#flake-transit)) Enforce the use of flakes for scripting operations.
...
### (on hold) Emphasis on flake over shell script wrappers
...
<sub><b>Note:</b> `nix copy` is for copying "closure" between two nix stores and cannot facilitate upstream caching, which substituter setup do; to fallback to next substituter if path doesn't exist in one. There isn't a way to pull paths from configured caches even if we know the paths beforehand.</sub>
...
There should be a way to build isolated directory with the filtered store path references only, or even an option to have upstream filtered paths (that are not in https://cache.nixos.org). Something like:
...
There are no perceived drawbacks in nix-cli improvements.

<p id="flake-transit">Flake enforcement is on hold, and if the necessary requirements are fulfilled by default.nix (typical, oldschool interfaces), it will get removed from RFC after checking all edge cases.</p>
